  ...
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/produtos_pool" && request.method === "POST") {
      try {
        ...
        if (existe) {
          return new Response(
            JSON.stringify({ ok: false, msg: "J\xE1 existe", id: existe.id }),
            { headers }
          );
        }
        await db.prepare(
